...
namespace AthernetCLI
{
    class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            node1 = new IP(IPAddress.Parse("192.168.1.2"), 1, playDeviceNumber:2, recordDeviceNumber:0, 2048 - 7);
            node2 = new IP(IPAddress.Parse("192.168.1.1"), 1, playDeviceNumber:1, recordDeviceNumber:2, 2048 - 7); // node2 is NAT
            // //Trace.Listeners.Add(new TextWriterTraceListener(Console.Out));
            //var watch = Stopwatch.StartNew();
            //
            Nat(IPAddress.Parse("10.20.212.86"));


            SendIcmp(Dns.GetHostEntry("china.zince.tech").AddressList[0]); // node1
            ReceiveUdp("192.168.1.2", null);
        ...
        private static void SendIcmpResponse(FileInfo input, IPAddress ipAddress)
        {
            // hear IcmpRequest from outside
            node1.StartReceive();
            node1.PacketAvailable += (sender, args) =>
            {
                if (args.Packet.TcpHeader is IcmpHeader icmpheader) // received ping request
                {
                    if (icmpheader.Type == IcmpType.EchoRequest)
                    {
        ...
        private static void SendIcmp(IPAddress ipAddress)
        {
            ...
            Console.WriteLine($"Sending to {ipAddress}...");
            for (int i = 0; i < 10; i++)
            {
                var bytes = new byte[16];
                new Random().NextBytes(bytes);
        ...
        private static void ReceiveUdp(string source, FileInfo output)
        {
            ...
            foreach (byte byte_contents in receivedBytes)
            {
                Debug.Write($"{byte_contents} "); 
            }
            node2.SendPacket(new Ipv4Packet
            {
                Header = new Ipv4Header
