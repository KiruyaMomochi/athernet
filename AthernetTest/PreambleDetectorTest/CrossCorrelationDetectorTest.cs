...
namespace AthernetTest.PreambleDetectorTest
{
    public class CrossCorrelationDetectorTest
    {
        ...
        {
            var preamble = new Athernet.Preambles.PreambleBuilders.WuPreambleBuilder(48000, 0.01f).Build();
            var detector = new Athernet.Preambles.PreambleDetectors.CrossCorrelationDetector(preamble);
