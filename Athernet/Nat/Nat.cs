    ...
    public class Nat
    {
        ...
        private string FilterString
        {
            get
            {
                ...
                {
                    return "dst port 12312";
                }
                
                // return "tcp and (" + string.Join(" or ", _filterPorts.Select(x => $"(dst port {x})")) + ")";
                return string.Join(" or ", _filterPorts.Select(x => $"(dst port {x})"));
            }
        }
        ...
        private void OnAthernetPacketAvailable(byte[] data)
        {
            ...
            var modifiedPacket = PacketBuilder.Build(DateTime.Now, ipV4Layer, tcpLayer, tcpDatagram.Payload.ExtractLayer());
